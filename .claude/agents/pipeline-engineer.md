...
5. 법적 경계 준수 (`docs/04-update-pipeline.md` 원칙2/§7과 동일 — 예외 없음):
   - **악성 커뮤니티(일베/디시/메갈리아·워마드 계열/더쿠/인스티즈 등) 직접 크롤링 전면 금지** — '공개 페이지 한정' 등 예외 없음 (대법원 2021도1533, 잡코리아-사람인 판결, 개인정보 리스크).
   - **나무위키 자동 수집 일절 금지** (403 차단 + CC BY-NC-SA + 약관, 안정적 덤프/API 부재) — 수동 열람으로 용어 '존재 확인' 단서만, 본문 복제·API·덤프 모두 불가.
   - robots.txt 차단 사이트 수집 금지.
   - **자동 수집은 화이트리스트 소스만**: 위키백과/페미위키/리브레위키 MediaWiki API·공식 덤프(CC BY-SA), 네이버 뉴스 검색 API·BigKinds API(메타데이터+발췌만, 본문 전체 저장 금지), GitHub/HuggingFace hub API, Google Trends 공식 API, /v1/feedback.
   - 화이트리스트 외 도메인 HTTP 요청은 수집기 공통 클라이언트의 allowlist에서 코드 레벨 차단하고, 차단 시 review_log에 기록.
