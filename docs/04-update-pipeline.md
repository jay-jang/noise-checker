...
- 처리: 제목/링크/날짜만 저장(본문 전체 저장 금지 — 저작권) → LLM이 기사에서 **문제가 된 표현·표식 추출** → 후보 엔트리 초안 생성(incident + term 후보 동시) → 검수 큐.
...
2. 체크리스트: ① origin 출처 ≥ 1 (청정 라이선스) ② 동음이의어 검토 + safe_contexts ③ severity 산정 근거 ④ 카테고리 ⑤ 법무 플래그(특정인 실명 관련 여부).
...
trigger (주간 정기 / urgent 핫픽스)
  ...
  → 오토마톤/패턴/이미지 인덱스 컴파일 → manifest(체크섬)
...
- **NC 자원 격리**: UnSmile/나무위키 등 비영리 자원은 별도 스토리지·평가 전용 플래그, 릴리스 아티팩트에 절대 미포함 (빌드 게이트에서 라이선스 필드 검사로 강제).
